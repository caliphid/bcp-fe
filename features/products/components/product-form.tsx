...
import { ProductType, CreateProductRequest, CreateProductVariantRequest } from "../../../types/product";
...
export function ProductForm({ businessUnits, productCategories, initialData }: ProductFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      sku: initialData?.sku || "",
      defaultHpp: initialData?.defaultHpp ? String(initialData.defaultHpp) : "0",
      defaultPrice: initialData?.defaultPrice ? String(initialData.defaultPrice) : "0",
      hasVariants: initialData ? (initialData.variants && initialData.variants.length > 0) : false,
  ...
  const handleGenerateVariants = () => {
    ...
    // Replace existing variants or append? Replacing is cleaner for "generation"
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (!data.hasVariants) {
        ...
        productPayload.defaultHpp = data.defaultHpp ? String(parseFloat(data.defaultHpp.replace(/\D/g, ""))) : "0";
        productPayload.defaultPrice = data.defaultPrice ? String(parseFloat(data.defaultPrice.replace(/\D/g, ""))) : "0";
      }
      ...
      if (initialData) {
        ...
        if (!productId) throw new Error("Failed to create product. ID not returned.");
      }
      ...
      if (data.hasVariants && data.variants && data.variants.length > 0) {
        const variantPromises = data.variants.map((v: any) => {
          const variantPayload: any = {
            ...
            sellingPrice: v.sellingPrice ? String(parseFloat(String(v.sellingPrice).replace(/\D/g, ""))) : undefined,
      ...
      if (initialData?.variants) {
        const currentVariantIds = new Set(data.variants?.filter((v: any) => v.id).map((v: any) => v.id) || []);
        const originalVariants = initialData.variants;
        const variantsToRemove = originalVariants.filter((v: any) => !currentVariantIds.has(v.id) && v.status === "ACTIVE");
        
        if (variantsToRemove.length > 0) {
          await Promise.all(variantsToRemove.map((v: any) => productApi.deactivateProductVariant(v.id)));
        }
      }
      ...
      toast.error(`Failed to ${initialData ? 'update' : 'create'} product completely. Check logs.`);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <h2 className="text-lg font-bold text-slate-800 mb-6">1. Product Detail</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <Input placeholder="e.g. Basic Edition" {...register("articleName")} />
          </div>
          ...
          <div className="space-y-2">
            <Label>Product Type <span className="text-rose-500">*</span></Label>
            <select
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <div className="flex items-start justify-between mb-6">
          <div>
            <h2 className="text-lg font-bold text-slate-800">2. Pricing & Variants</h2>
            <p className="text-sm text-slate-500 mt-1">
              Configure product pricing. Enable variants if this product has multiple sizes, colors, or options.
            </p>
          </div>
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 px-4 py-2 rounded-lg">
            ...
            />
            <Label htmlFor="hasVariants" className="font-semibold cursor-pointer">
        ...
        {hasVariants && (
          <div className="space-y-6">
            ...
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4">
              <h3 className="text-sm font-semibold text-indigo-900 mb-3">Auto-Generate Variant Combinations</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <Label className="text-xs text-indigo-700">Colors (comma separated)</Label>
                  <Input 
                ...
                <div className="space-y-1">
                  <Label className="text-xs text-indigo-700">Sizes (comma separated)</Label>
                  <Input 
              ...
              <div className="mt-4 flex justify-end">
                <Button type="button" onClick={handleGenerateVariants} className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm">
            ...
            {fields.length > 0 && (
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-slate-800">Bulk Apply to All Variants</h3>
                  <span className="text-xs text-slate-500">Apply specific pricing or stock to all generated rows below</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                  <div className="space-y-1">
                    ...
                    <Input 
                      ...
                      value={bulkUnitCost} 
                      onChange={(e) => setBulkUnitCost(formatInputMoney(e.target.value))}
                  ...
                  <div className="space-y-1">
                    ...
                    <Input 
                      ...
                      onChange={(e) => setBulkSellingPrice(formatInputMoney(e.target.value))}
                      className="bg-white text-sm h-9"
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input 
                      ...
                      placeholder="0" 
                      value={bulkMinStock} 
                      onChange={(e) => setBulkMinStock(e.target.value ? Number(e.target.value) : "")}
                      className="bg-white text-sm h-9"
                    />
                  </div>
                  <div>
                    <Button type="button" variant="outline" onClick={handleBulkApply} className="w-full h-9 border-slate-300">
                      Apply to All
                    </Button>
                  </div>
                </div>
            ...
            {errors.variants?.root && (
              <p className="text-sm text-red-500 font-medium bg-red-50 p-2 rounded">{errors.variants.root.message}</p>
            )}

            {fields.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end p-4 bg-slate-50 border border-slate-200 rounded-xl relative hover:border-slate-300 transition-colors">
                ...
                <div className="md:col-span-2 space-y-2">
                  <Label className="text-xs">SKU <span className="text-rose-500">*</span></Label>
                  <Input
                    className="h-9 text-sm"
                    placeholder="e.g. TS-BLK-S"
                    {...register(`variants.${index}.sku` as const)}
                  />
                  {errors.variants?.[index]?.sku && <p className="text-[10px] text-red-500">{errors.variants[index]?.sku?.message}</p>}
                </div>
                
                <div className="md:col-span-2 space-y-2">
                  <Label className="text-xs">Color <span className="text-rose-500">*</span></Label>
                  <Input
                    className="h-9 text-sm"
                    placeholder="e.g. Black"
                    {...register(`variants.${index}.color` as const)}
                  />
                  {errors.variants?.[index]?.color && <p className="text-[10px] text-red-500">{errors.variants[index]?.color?.message}</p>}
                </div>

                <div className="md:col-span-2 space-y-2">
                  <Label className="text-xs">Size <span className="text-rose-500">*</span></Label>
                  <Input
                    className="h-9 text-sm"
                    placeholder="e.g. S, M, L"
                    {...register(`variants.${index}.size` as const)}
                  />
                  {errors.variants?.[index]?.size && <p className="text-[10px] text-red-500">{errors.variants[index]?.size?.message}</p>}
                </div>

                <div className="md:col-span-2 space-y-2">
                  <Label className="text-xs">Unit Cost (HPP) <span className="text-rose-500">*</span></Label>
                  <Input
                    className="h-9 text-sm"
                    placeholder="Rp 0"
                    {...register(`variants.${index}.unitCost` as const)}
                    onChange={(e) => {
                      setValue(`variants.${index}.unitCost`, formatInputMoney(e.target.value));
                    }}
                  />
                  {errors.variants?.[index]?.unitCost && <p className="text-[10px] text-red-500">{errors.variants[index]?.unitCost?.message}</p>}
                </div>
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      setValue(`variants.${index}.sellingPrice`, formatInputMoney(e.target.value));
                    }}
                  />
                </div>
                ...
                <div className="md:col-span-1 space-y-2">
                  ...
                  <Input
                    ...
                    min="0"
                    className="h-9 text-sm"
                    {...register(`variants.${index}.minimumStock` as const, { valueAsNumber: true })}
                  />
                </div>
            ...
            <Button
              ...
              onClick={() => append({ sku: "", color: "", size: "", unitCost: "0", sellingPrice: "0", minimumStock: 0 })}
            >
        ...
        <div className="flex justify-end gap-4 mt-8 pt-4 border-t border-slate-200">
          ...
          <Button type="submit" disabled={isSubmitting} className="min-w-[140px] shadow-primary-500/30 shadow-md">
            {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : (initialData ? "Save Changes" : "Create Product")}
          </Button>
        </div>
      </div>
    </form>
