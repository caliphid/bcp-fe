...
interface VoidInstallmentPaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  paymentCode: string;
...
export function VoidInstallmentPaymentModal({
  isOpen,
  onClose,
  paymentCode,
  onConfirm,
}: VoidInstallmentPaymentModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        Array.isArray(msg) ? msg.join(", ") : msg || "Failed to void payment",
      );
    } finally {
      setLoading(false);
    }
      ...
      <DialogContent className="sm:max-w-[425px] bg-white p-0 overflow-hidden border-0 shadow-2xl rounded-2xl">
        <div className="bg-rose-50 p-6">
          ...
          <p className="text-rose-600/80 text-sm text-center mt-2 font-medium">
            Apakah Anda yakin ingin membatalkan pembayaran <br />
            <span className="text-rose-700 font-bold">{paymentCode}</span>?
          </p>
          <p className="text-slate-600 text-xs text-center mt-2">
            Tindakan ini akan mengembalikan nominal sisa cicilan dan juga
            membatalkan transaksi cashflow yang tertaut.
          </p>
        </div>
